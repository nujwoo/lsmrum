...
//    return cleaned pageId
    @SuppressWarnings("rawtypes")
    public int vacuumClean(int pageId, IRTreeLeafFrame leafFrame, IRTreeInteriorFrame interiorFrame,
    		UpdateMemo<Integer> memo) throws Exception {
        return cleanNextPage(pageId, interiorFrame, leafFrame, memo);
    }


    public int cleanNextPage(int pageId, IRTreeInteriorFrame interiorFrame, IRTreeLeafFrame leafFrame,
    		UpdateMemo<Integer> memo) throws Exception{
    	...
    	try {
    		...
    		MultiComparator cmp = MultiComparator.create(cmpFactories);
			rightPageId = cleanFirstPage(rootPage, interiorFrame, leafFrame, cmp, memo);
			return rightPageId;
    	}
    	...
    	if(node != null) {
	    	...
	    	try {
	    		...
	    		if (interiorFrame.isLeaf()) {
	    			...
	    			rightPageId = leafFrame.getRightPage();
	//        		clean
	        		((RTreeNSMLeafFrame)leafFrame).cleanFrame(memo);
	    		}
    ...
    public int cleanFirstPage(int pageId, IRTreeInteriorFrame interiorFrame, IRTreeLeafFrame leafFrame,
    			MultiComparator cmp, UpdateMemo<Integer> memo) throws Exception {
    	int returnId;
...
//    		clean
    		...
    		returnId = cleanFirstPage(((RTreeNSMInteriorFrame) (interiorFrame)).getFirstChild(cmp),
    								interiorFrame, leafFrame, cmp, memo);
    	}
    ...
    private void insertTuple(ICachedPage node, int pageId, ITupleReference tuple, RTreeOpContext ctx, boolean isLeaf, UpdateMemo<Integer> memo)
            throws HyracksDataException {
        boolean succeeded = false;
        FrameOpSpaceStatus spaceStatus;
        if (!isLeaf) {
            ...
            case SUFFICIENT_CONTIGUOUS_SPACE: {
                try {
                    if (!isLeaf) {
                        ...
                        ctx.getLeafFrame().insert(tuple, -1);
                        if(node.needsClean()) {
                        	((RTreeNSMLeafFrame)ctx.getLeafFrame()).cleanFrame(memo);
                        }
                    }
                    succeeded = true;
                } finally {
    ...
    public class RTreeAccessor implements ITreeIndexAccessor {
        ...
        private final int vacuumThreshold = CleaningConfig.VACUUM_THRESHOLD;
        private int cleanedPage;
        ...
        public String printTree() {
        	...
        	try {
				...
				return rtree.printTree(ctx.getLeafFrame(), ctx.getInteriorFrame(), fieldSerdes);
			} catch (Exception e) {
        ...
        public void vacuumClean(UpdateMemo<Integer> memo){
        	ctx.reset();
        	try {
        		this.cleanedPage = rtree.vacuumClean(this.cleanedPage, ctx.getLeafFrame(), ctx.getInteriorFrame(), memo);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        ...
        public void insertVacuumClean(ITupleReference tuple, UpdateMemo<Integer> memo) throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
            insert(tuple, ctx);
            vacuumCount++;
            if(vacuumCount == vacuumThreshold) {
            	vacuumCount = 0;
            	vacuumClean(memo);
            }
        }

        public void insertClean(ITupleReference tuple, UpdateMemo<Integer> memo) throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
            insertClean(tuple, ctx, memo);
        }

        public void insertCleanVacuumClean(ITupleReference tuple, UpdateMemo<Integer> memo) throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
            insertClean(tuple, ctx, memo);
            vacuumCount++;
            if(vacuumCount == vacuumThreshold) {
        ...
        private void insertClean(ITupleReference tuple, IIndexOperationContext ictx, UpdateMemo<Integer> memo)
        		throws HyracksDataException {
            RTreeOpContext ctx = (RTreeOpContext) ictx;
            int tupleSize = Math.max(ctx.getLeafFrame().getBytesRequiredToWriteTuple(tuple),
                    ctx.getInteriorFrame().getBytesRequiredToWriteTuple(tuple));
            ...
            try {
                ...
                int pageId = ctx.getPathList().getLastPageId();
                ctx.getPathList().moveLast();
                insertTuple(leafNode, pageId, ctx.getTuple(), ctx, true, memo);
