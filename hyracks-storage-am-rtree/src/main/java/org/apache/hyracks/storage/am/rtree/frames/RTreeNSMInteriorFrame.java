...
public class RTreeNSMInteriorFrame extends RTreeNSMFrame implements IRTreeInteriorFrame {
    ...
    public int getFirstChild(MultiComparator cmp) {
    	frameTuple.setFieldCount(cmp.getKeyFieldCount());
    	int tupleOff = slotManager.getTupleOff(slotManager.getSlotOff(0));
    	frameTuple.resetByTupleOffset(buf.array(), tupleOff);
    	return IntegerPointable.getInteger(buf.array(), frameTuple.getFieldStart(frameTuple.getFieldCount() - 1)
                        + frameTuple.getFieldLength(frameTuple.getFieldCount() - 1));
    }
