...
import org.apache.hyracks.storage.am.common.updatememo.UMEntity;
...
public class LSMRTree extends AbstractLSMRTree {
      ...
      BTreeAccessor memBTreeAccessor =
        ...
        try {
            try {
                ...
                if(isLocalUM()) {
                	mergeUMLocalToGlobal(flushingComponent);
                }
            } finally {
                try {
                    ...
                    if (USE_LOCAL_UM) {
                    	flushingComponent.getLocalUM().reset();
                    }
                    else {
                    }
                }
        ...
        UpdateMemoConfig.sumFlushTime += (end-start);
        return component;
    }

    private void mergeUMLocalToGlobal(LSMRTreeMemoryComponent flushingComponent) {
    	for (Entry<Integer, UMEntity<Integer>> entry : flushingComponent.getLocalUM().getEntrySet()) {
    	    this.globalUM.insertOrUpdate(entry.getKey(), entry.getValue());
    	}
    }
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        // RTree.
                    	if(CLEAN_UPON_FLUSHING) {
                    		isEmpty.setValue(filteredFill(flushingComponent, rtreeScanCursor, rTreeTupleSorter));
                    	}
                    	else {
    ...
    private boolean filteredFill(LSMRTreeMemoryComponent flushingComponent, RTreeSearchCursor rtreeScanCursor, TreeTupleSorter rTreeTupleSorter)
            throws HyracksDataException {
        boolean isEmpty = true;
        while (rtreeScanCursor.hasNext()) {
            rtreeScanCursor.next();

            ITupleReference tuple = rtreeScanCursor.doGetTuple();
            byte[] tupleData = tuple.getFieldData(0);
        	int ki = tuple.getFieldCount()-2;
        	int ti = tuple.getFieldCount()-1;

        	boolean isOld;
        	if(USE_LOCAL_UM) {
        		isOld = flushingComponent.getLocalUM().cleanEntity(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
            			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
        	}
        	else {
        		isOld = globalUM.cleanEntity(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
            			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
        	}
    ...
    public ILSMDiskComponent doMerge(ILSMIOOperation operation) throws HyracksDataException {
        ...
        try {
            ...
            if(CLEAN_UPON_MERGING) {
            	componentBulkLoader = filteredLoadMergeBulkLoader(mergeOp, cursor, mergedComponent);
            }
            else {
        ...
        long end = System.currentTimeMillis();
        UpdateMemoConfig.sumMergeTime += (end-start);
...
//        lsmHarness.
    ...
    private ILSMDiskComponentBulkLoader filteredLoadMergeBulkLoader(LSMRTreeMergeOperation mergeOp, IIndexCursor cursor,
            ILSMDiskComponent mergedComponent) throws HyracksDataException {
        ILSMDiskComponentBulkLoader componentBulkLoader = null;
        boolean abort = true;
        ISearchPredicate rtreeSearchPred = new SearchPredicate(null, null);
        ILSMIndexOperationContext opCtx = ((LSMRTreeSortedCursor) cursor).getOpCtx();
        LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) memoryComponents.get(currentMutableComponentId.get());
        search(opCtx, cursor, rtreeSearchPred);
        try {
            try {
                // In case we must keep the deleted-keys BTrees, then they must be merged
                ...
                while (cursor.hasNext()) {
                    ...
                    boolean isOld;
                	if(USE_LOCAL_UM) {
                		isOld = tComp.getLocalUM().cleanEntity(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
                    			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
    ...
    public void modify(IIndexOperationContext ictx, ITupleReference tuple) throws HyracksDataException {
        ...
        else { // ctx.getOperation() == IndexOperation.DELETE
        	byte[] tupleData = indexTuple.getFieldData(0);

        	int ki = tuple.getFieldCount()-2;
        	int ti = tuple.getFieldCount()-1;

        	if(USE_LOCAL_UM) {
        		LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
        		tComp.getLocalUM().insertOrUpdate(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
            			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());

        	}
        	else {
        		globalUM.insertOrUpdate(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
            			ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
        	}
        	if(ctx.getOperation() == IndexOperation.UPDATE) {
        		if(UpdateMemoConfig.CLEAN_UPON_UPDATE) {
        			if(UpdateMemoConfig.VACUUM_CLEAN) {
        				if(USE_LOCAL_UM) {
            				LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
            				((RTreeAccessor)ctx.getCurrentMutableRTreeAccessor()).insertCleanVacuumClean(indexTuple, tComp.getLocalUM());
            			}
            			else {
            				((RTreeAccessor)ctx.getCurrentMutableRTreeAccessor()).insertCleanVacuumClean(indexTuple, globalUM);
            			}
        			}
        			else {
        				if(USE_LOCAL_UM) {
            				LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
            				((RTreeAccessor)ctx.getCurrentMutableRTreeAccessor()).insertClean(indexTuple, tComp.getLocalUM());
            			}
            			else {
            				((RTreeAccessor)ctx.getCurrentMutableRTreeAccessor()).insertClean(indexTuple, globalUM);
            			}
        			}
        			...
        			if(UpdateMemoConfig.VACUUM_CLEAN) {
        				if(USE_LOCAL_UM) {
            				LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
            				((RTreeAccessor)ctx.getCurrentMutableRTreeAccessor()).insertVacuumClean(indexTuple, tComp.getLocalUM());
            			}
            			else {
            				((RTreeAccessor)ctx.getCurrentMutableRTreeAccessor()).insertVacuumClean(indexTuple, globalUM);
            			}
        			}
        			else {
        				ctx.getCurrentMutableRTreeAccessor().insert(indexTuple);
	...
	@Override
	public UpdateMemo<Integer> getGlobalUM() {
		return globalUM;
	}
}
