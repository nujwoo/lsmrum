...
public abstract class AbstractLSMRTree extends AbstractLSMIndex implements ITreeIndex {
    ...
    public AbstractLSMRTree(IIOManager ioManager, List<IVirtualBufferCache> virtualBufferCaches,
            ...
            UpdateMemo<Integer> globalUM) throws HyracksDataException {
        super(ioManager, virtualBufferCaches, diskBufferCache, fileManager, bloomFilterFalsePositiveRate, mergePolicy,
                ...
                rtreeFields, ITracer.NONE, globalUM);
...
//          if we use only one global
            LSMRTreeMemoryComponent mutableComponent;
            if(isLocalUM()) {
            	UpdateMemo<Integer> localUM = new UpdateMemo<Integer>();
                mutableComponent = new LSMRTreeMemoryComponent(this, memRTree, memBTree,
                        virtualBufferCache, filterHelper == null ? null : filterHelper.createFilter(), localUM);
            }
            else {
            	mutableComponent = new LSMRTreeMemoryComponent(this, memRTree, memBTree,
                        virtualBufferCache, filterHelper == null ? null : filterHelper.createFilter(), null);
            }
