...
//        currentCursor.
        while (currentCursor < numberOfTrees) {
            while (rtreeCursors[currentCursor].hasNext()) {
                ...
                byte[] tupleData = currentTuple.getFieldData(0);
                int ki = currentTuple.getFieldCount()-2;
            	int ti = currentTuple.getFieldCount()-1;
            	boolean killerTupleFound = false;
                if(lsmHarness.useLocalUM()) { // local UM
                	LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) opCtx.getComponentHolder().get(0);
                	if(tComp.getLocalUM().hasNewerEntity(
            				ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ki), currentTuple.getFieldLength(ki)).getInt(),
                			ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ti), currentTuple.getFieldLength(ti)).getInt())){
                		killerTupleFound = true;
                	}
                	else {
                		if(lsmHarness.getGlobalUM().hasNewerEntity(
                    			ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ki), currentTuple.getFieldLength(ki)).getInt(),
                    			ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ti), currentTuple.getFieldLength(ti)).getInt())) {
                    		killerTupleFound = true;
                    	}
